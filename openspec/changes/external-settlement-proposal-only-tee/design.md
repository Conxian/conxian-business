...
To ensure cross-implementation stability, `trigger_id` MUST be derived exactly as specified in [spec.md §1.7 (Idempotency / replay protection)](specs/external-settlement-proposal-only-tee/spec.md) (illustrative mental model only: `hexLower(sha256(utf8("external-settlement-trigger:v1") || utf8(JCS({"rail": canonical_rail, "normalized_settlement_hash": normalized_settlement_hash}))))`, where `canonical_rail` is exactly one of `ISO20022`, `PAPSS`, or `BRICS`).
