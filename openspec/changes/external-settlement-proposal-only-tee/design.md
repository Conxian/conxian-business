...
- `settlement_identifiers.transaction_identifiers` MUST be present and MUST be a JSON object whose values are strings; if it is missing, not an object, or any value is non-string, canonicalization MUST fail for the corresponding settlement transaction.
...
- `envelope_identifiers.tx_index` is a non-negative integer (required). Any other optional `settlement_identifiers.envelope_identifiers.*` values are treated as absent if invalid and MUST NOT invalidate the settlement transaction.
