...
Unrecognized keys under `settlement_identifiers.envelope_identifiers` MUST be treated as non-consensus metadata. Provided that values under those keys satisfy the structural and leaf-type constraints in this section, implementations MUST ignore those keys for external-settlement trigger purposes (including validity decisions and any trigger-critical computation such as `normalized_settlement_hash`), and their mere presence MUST NOT cause the settlement transaction to be treated as invalid. If a value under any unrecognized `settlement_identifiers.envelope_identifiers` key violates a structural or leaf-type constraint in this section, the settlement transaction MUST be treated as invalid as specified above.
