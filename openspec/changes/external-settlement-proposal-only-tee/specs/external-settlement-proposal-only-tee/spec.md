...
- Separately, before any canonical-vs-hint comparison or any trigger-critical computation, the TEE MUST treat any optional `settlement_identifiers.envelope_identifiers` fields (i.e., any keys other than `tx_index`) whose values individually violate any field-specific canonicalization rule for that key, if defined, as absent. Optional `settlement_identifiers.envelope_identifiers` fields MUST NOT be discarded in order to bypass the maximum nesting depth, maximum leaf-field count, or RFC 8785 JCS size bounds; any violation of those global bounds MUST cause the TEE to refuse to produce a successful attestation.
...
For both the TEE-derived canonical `settlement_identifiers` object and any host-supplied `settlement_identifiers` hints, implementations MUST enforce the maximum nesting depth, maximum leaf-field count, and RFC 8785 JCS size bounds in this section on the parsed object before discarding any optional `settlement_identifiers.envelope_identifiers` fields. Before computing `normalized_settlement_hash` (and before any canonical-vs-hint comparison), implementations MUST treat any optional `settlement_identifiers.envelope_identifiers` fields (i.e., any keys other than `tx_index`) whose values individually violate any field-specific canonicalization rule for that key, if defined, as absent. Optional `settlement_identifiers.envelope_identifiers` fields MUST NOT be discarded in order to bypass the maximum nesting depth, maximum leaf-field count, or RFC 8785 JCS size bounds; any violation of those global bounds MUST be treated as a permanent validation failure.
...
11. `settlement_identifiers` (canonical or host hint) violates the structural/leaf-type constraints in §2.1.1 → TEE refuses to produce a successful attestation.
