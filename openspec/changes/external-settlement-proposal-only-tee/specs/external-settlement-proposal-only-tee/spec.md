...
If `oracle_verification` includes `oracle_proof_bytes_b64`, its character length MUST be ≤ 21846 (the maximum base64url length corresponding to 16384 raw bytes with padding removed), and every component that base64url-decodes `oracle_proof_bytes_b64` MUST enforce this bound before decoding. Decoding it MUST yield exactly the `raw_oracle_proof_bytes` used both to verify the oracle authenticity proof and to compute `oracle_proof_digest`.
...
`envelope_identifiers.tx_index` MUST satisfy the field requirements above. If `envelope_identifiers.tx_index` is missing or fails these requirements, the corresponding settlement transaction MUST be treated as invalid for external-settlement trigger purposes and MUST NOT produce a `normalized_settlement_hash` or `SovereignProposal`. Any other `settlement_identifiers.envelope_identifiers` keys are optional metadata; malformed, unrecognized, or otherwise invalid values for those optional envelope identifiers MUST be ignored/omitted and MUST NOT cause the settlement transaction to be treated as invalid for external-settlement trigger purposes.

For both the TEE-derived canonical `settlement_identifiers` object and any host-supplied `settlement_identifiers` hints, implementations MUST drop any optional `settlement_identifiers.envelope_identifiers` fields (i.e., any keys other than `tx_index`) whose values are malformed, unrecognized, or otherwise invalid before enforcing the structural/leaf-type constraints, maximum depth/leaf-count bounds, or RFC 8785 JCS size bounds in this section, and before computing `normalized_settlement_hash`. After this removal step, the remaining object MUST satisfy all constraints in this section.
