...
- After parsing, the hint object MUST satisfy the structural and leaf-type constraints in this section, have max nesting depth ≤ 8, contain ≤ 128 leaf fields, and when serialized using RFC 8785 JCS (UTF-8) MUST be ≤ 16384 bytes. The TEE MUST enforce the depth, leaf-count, and structural/leaf-type constraints during validation traversal and MUST abort traversal as soon as it can determine any such bound is exceeded. The JCS-serialized size bound MUST also be enforced before performing any canonical-vs-hint comparison traversal or other expensive processing. If any constraint or bound is violated, the TEE MUST refuse to produce a successful attestation.
