...
- After parsing, the hint object MUST satisfy the structural and leaf-type constraints in this section, have max nesting depth ≤ 8, contain ≤ 128 leaf fields, and when serialized using RFC 8785 JCS (UTF-8) MUST be ≤ 16384 bytes. The TEE MUST enforce these bounds during validation traversal (before performing any canonical-vs-hint comparison traversal or other expensive processing) and MUST abort traversal as soon as any bound is exceeded. If any constraint or bound is violated, the TEE MUST refuse to produce a successful attestation.
