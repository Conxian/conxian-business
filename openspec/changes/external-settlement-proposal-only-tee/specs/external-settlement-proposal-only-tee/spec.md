...
- Separately, before any canonical-vs-hint comparison or any trigger-critical computation, the TEE MUST treat any optional `settlement_identifiers.envelope_identifiers` fields (i.e., any keys other than `tx_index`) whose values individually violate any field-specific canonicalization or validation requirement for that key, if defined, as absent. Any canonical-vs-hint comparison MUST be performed against this post-processing view of the hint object. Optional `settlement_identifiers.envelope_identifiers` fields MUST NOT be discarded in order to bypass the maximum nesting depth, maximum leaf-field count, or RFC 8785 JCS size bounds; any violation of those global bounds MUST cause the TEE to refuse to produce a successful attestation.
