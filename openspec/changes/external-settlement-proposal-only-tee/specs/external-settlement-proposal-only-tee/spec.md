...
Any other `settlement_identifiers.envelope_identifiers` keys are optional metadata. If an optional envelope identifier is present and satisfies the structural and leaf-type constraints in this section but fails any additional field-specific type or validation requirement defined by this spec, implementations MUST treat that value as if the corresponding field were absent and MUST NOT treat the settlement transaction as invalid solely because of that field. If an optional envelope identifier violates any structural or leaf-type constraint in this section, the settlement transaction MUST be treated as invalid as specified above.

Unrecognized keys under `settlement_identifiers.envelope_identifiers` MUST be ignored for all consensus-critical purposes of `external-settlement-trigger:v1` (including validity decisions and any trigger-critical computation such as `normalized_settlement_hash`). Provided that values under those keys satisfy the structural and leaf-type constraints in this section, their mere presence MUST NOT cause the settlement transaction to be treated as invalid. If a value under any unrecognized `settlement_identifiers.envelope_identifiers` key violates a structural or leaf-type constraint in this section, the settlement transaction MUST be treated as invalid as specified above.
