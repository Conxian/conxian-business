...
- `burn_block_height` is the canonical field name for burn-block anchoring (Stacks burnchain block height anchored to Bitcoin). Where a keyed export (for example JSON object keys or CSV headers) previously exposed `burn-block-height` and the alias remains representable and operationally safe, implementations **MUST** emit both `burn_block_height` and the deprecated alias `burn-block-height` for the duration of the explicitly documented deprecation window defined in the dataset schema. Where `burn-block-height` is not representable or is operationally unsafe in a given representation (for example SQL column identifiers), implementations **MUST NOT** emit the alias and instead **MUST** document, in the dataset schema or a version-controlled mapping file it references, the deterministic mapping from `burn-block-height` to `burn_block_height`, subject to the same deprecation window.
