...
These tracks **MUST** enforce the canonical `snake_case` egress schema (including deprecated aliases and/or replacement datasets for field renames) and publish deterministic renderer mappings (for example ISO 20022) in version control alongside each dataset schema (see spec: Export field naming conventions).
