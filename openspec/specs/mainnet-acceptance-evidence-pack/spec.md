...
- Canonical remote push URL(s): `git remote get-url --push --all <canonical-remote>` (record output verbatim; preserve line order. If all push URL(s) are identical to fetch URL(s), you may record `Same as fetch URL(s) above` instead of duplicating the list; if output contains embedded credentials, remediate the git remote configuration and re-run capture before recording)
...
The configured canonical remote URL(s) MUST NOT include embedded credentials (tokens, `user:pass@`, etc.). If any are discovered during capture, remediate the git remote configuration to remove embedded credentials and re-run the URL capture commands above. If embedded credentials were captured in any drafts, redact them immediately. The final evidence pack MUST contain only credential-free URL(s).
...
- Canonical remote fetch URL(s) (e.g. from `git remote get-url --all <canonical-remote>`; fallback: `git config --get-all remote.<canonical-remote>.url`; preserve line order; MUST NOT include embedded credentials; if any are present, remediate the git remote configuration and re-run capture before recording):
  - `<url>` (one line per command output line; preserve order)
- Canonical remote push URL(s) (e.g. from `git remote get-url --push --all <canonical-remote>`; fallback: `git config --get-all remote.<canonical-remote>.pushurl`; preserve line order; MUST NOT include embedded credentials; if any are present, remediate the git remote configuration and re-run capture before recording):
