...
These SHAs and the capture timestamp MUST be recorded **before merging** the promotion PR (while the PR is open). First run `git fetch origin main staged` to ensure the `origin/*` refs are current, then record the values using the commands above.
