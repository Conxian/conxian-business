...
- **CI/CD Production Alignment**: Audited all 15 CI workflow files. Fixed `actions/checkout@v6`â†’`v4` in `deploy-docs`, `gateway-cloud-run`, `showcase-dapp-deploy`. Added submodule token + recursive checkout to `gateway-cloud-run` and `sovereign-guard`. Upgraded `sovereign-guard` from `setup-python@v5`â†’`v6`. Fixed `gemini-plan-execute` copy-paste error (`workflow_name: 'gemini-invoke'`â†’`gemini-plan-execute`).
