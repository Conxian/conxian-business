...
3. **CI Guardrails (scoped):** Enforce a scoped contamination scan in CI for `main`/`staged` that rejects non-production patterns in production source trees. Use `docs/BRANCHING_AND_PROMOTION_POLICY.md` as the canonical source for the scan scope, exclusions, and patterns.
