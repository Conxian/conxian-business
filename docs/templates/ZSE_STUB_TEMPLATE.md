...
- Link 2: replace `[ISSUE_KEY]` with the doc-specific Linear issue owning the canonical content (preferred; create one if missing).
  - If you cannot create the issue before merging, keep the `https://linear.app/conxian-labs/issue/[ISSUE_KEY]` placeholder link in place temporarily and replace `[ISSUE_KEY]` as soon as a canonical Linear issue exists.
