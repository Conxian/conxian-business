...
- **Authority / source of truth** (left-to-right): `Protocol → Nexus → Gateway → UI/Wallet`
- **Code / build-time dependencies (imports)** (each imports the module to its right): `UI/Wallet → Gateway → Nexus → Protocol`
