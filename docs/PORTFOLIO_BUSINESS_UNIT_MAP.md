...
Today, this page is the canonical human-readable portfolio map; machine-readable BOS audit outputs produced locally under `./conxian-business/.generated/` should be treated as derived outputs. When this mapping changes, ensure any intentional BOS runtime/audit updates remain consistent with it. This repository does not currently provide a committed `docs/PORTFOLIO_MANIFEST_generated.json` generator, so that filename must not be claimed as an existing artifact. The canonical process is to update the committed gitlinks and `.gitmodules`, reflect the same set in this map and `docs/REPO_PORTFOLIO.md`, then run `python3 scripts/verify_submodule_integrity.py`. Market-specific checks explicitly opt into the intentionally pinned market checkout with `git -c submodule.conxian-market.update=checkout submodule update --init conxian-market`.
