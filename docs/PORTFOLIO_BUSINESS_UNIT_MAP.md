...
- **Code / build-time dependencies (who depends on whom):** `UI/Wallet → Gateway → Nexus → Protocol` — authority flows from Protocol out to UI/Wallet, but code dependencies are only allowed in this opposite direction: each layer may depend only on the next one closer to Protocol (UI/Wallet → Gateway, Gateway → Nexus, Nexus → Protocol), so higher layers compose lower ones without redefining sources of truth.
