...
- The `auto-sync-submodules` workflow ([`.github/workflows/auto-sync-submodules.yml`](../.github/workflows/auto-sync-submodules.yml)) is the intentional exception: it updates only top-level submodule gitlinks and uses `git submodule update --remote` internally to create reviewable PRs that update pinned gitlinks. It sets `submodule.<name>.branch` via an explicit override when present, or else the first available of `main`/`master`, and then syncs nested submodules to the pins in the updated top-level commits. It must not depend on remote `HEAD`.
