...
Contributors should not edit generated BOS state artifacts under `./conxian-business/` by hand (for example: the generated audit manifest at `./conxian-business/.generated/AUDIT_MANIFEST.json`). The tracked `./conxian-business/AUDIT_MANIFEST.json` is a public-safe stub; it is **not** regenerated by `transparency_custodian.py`, and the canonical audit details are maintained in the Conxian Linear workspace.
...
- If your change affects portfolio wiring or BOS state inputs (for example: pinned submodule gitlinks (including adding/removing submodules), the mapping tables in this document (ecosystem submodule rows / BOS-native asset paths), or the generator source under `./conxian-business/`), re-run the generator command above (do **not** edit generated artifacts under `./conxian-business/.generated/` by hand; `./conxian-business/AUDIT_MANIFEST.json` remains a manually maintained public-safe stub) and **commit** any updated derived artifacts that are tracked in git in the same PR.
...
- The `auto-sync-submodules` workflow ([`.github/workflows/auto-sync-submodules.yml`](../.github/workflows/auto-sync-submodules.yml)) is the intentional exception: it updates only top-level submodule gitlinks and uses `git submodule update --remote` internally to create reviewable PRs that update pinned gitlinks. It sets `submodule.<name>.branch` via an explicit override when present (configured in the `PREFERRED_BRANCHES` map in the workflow), or else the first available of `main`/`master`, and then syncs nested submodules to the pins in the updated top-level commits. It must not depend on remote `HEAD`.
