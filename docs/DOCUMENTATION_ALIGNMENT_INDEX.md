...
- **Internal-only (Linear-only canonical)**: strategic / operational / security-adjacent material whose full text must live only in the Conxian Linear workspace (Linear is canonical). If a repo path must exist for link integrity, it must be represented as a `Public-safe stub (canonical in Linear)` pointer stub.
