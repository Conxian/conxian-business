...
| **W3: Analytics phase-out (proof/visual-proof datasets)** | Replace Supabase-backed analytics with a verifiable, rebuildable derived dataset layer. | Supabase analytics (phase-out path) | High | Medium | High | Depends on checkpoint discipline + a target-state decision for analytics (see [SAB-DS-002](./SAB_DATASTORE_DECISION_LOG.md#L23)). |
...
3. Analytics target-state is still open ([SAB-DS-002](./SAB_DATASTORE_DECISION_LOG.md#L23)), so it must not contaminate Wave 1 sequencing.
