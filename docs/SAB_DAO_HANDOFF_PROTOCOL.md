...
| **H-3** | Only for two-step contracts: from the designated SAB authority, accept/claim the pending owner/admin update (e.g., `claim-ownership`, `accept-ownership`). N/A for single-step transfers. | SAB Signers | Confirm final owner/admin update. |
