...
| SAB-DS-005 | Governance + audit ledger (append-only, queryable) | Fluree | Accepted | Query layer for append-only governance records (resolutions, policy bundles, audit trails) as JSON-LD. MUST be derived-only: every correctness-relevant record is anchored on-chain by content hash and validated at runtime. MUST remain ZSE-safe (no key material; legal/compliance content is metadata + hashes only). See `docs/SAB_IMMUTABLE_GOVERNANCE_RECORD_ARCHITECTURE.md` for domain classification and fail-closed requirements. | Botshelo Mokoka | 2026-05-04 (update after governance-ingress + policy-gate interfaces are defined) |
