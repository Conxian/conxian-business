...
- **Contamination Guard**: CI suites on `main` and `staged` must scan for and reject non-production patterns, but the scan must be explicitly scoped to avoid false positives.
