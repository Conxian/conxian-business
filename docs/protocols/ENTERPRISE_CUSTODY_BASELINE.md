...
- **Time-lock (baseline)**:
  ...
  - If a later classification event records a policy hash/version that requires a stricter minimum delay for the effective risk classification (even if its risk category does not change), a new immutable time-lock-start reference MUST be appended as part of that classification event (not mutating any prior reference) and the time-lock window for that stricter delay MUST be restarted and measured from that new time-lock-start reference. Enforcement MUST use the most recently appended time-lock-start reference for the strictest effective risk classification when determining entry into (and maintaining) the execution-ready state.
