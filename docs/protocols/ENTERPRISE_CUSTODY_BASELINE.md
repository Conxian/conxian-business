...
- **Execution-eligible state**: the point in the approval lifecycle where all required approvals are recorded, all policy checks (including risk classification) have passed, and the action is ready to enter any required time-lock window; after all applicable time-lock windows have elapsed under the effective risk classification, the only remaining step is to execute/sign/broadcast the action. Once a request enters the execution-eligible state, the request payload, policy hash/version, and approval set MUST be treated as immutable; any change to these fields MUST be expressed as a new request that re-enters the approval lifecycle. The risk-classification snapshot that justified the transition MUST be recorded immutably, but additional classification events MAY be appended later as new records without mutating that snapshot; effective risk classification MUST follow the rules in **High-risk threshold**.
...
- **Quorum**: any key that can (a) move value or (b) mutate the control plane MUST enforce threshold approval (minimum 2-of-3; target 3-of-5 where liveness allows) for any value movement request classified as `high-risk` under policy (including any value movement that meets an amount-based high-risk predicate for that rail/asset) and for all production control-plane mutations (including key rotations, deploy/upgrade, identity/authorization changes, and policy changes).
...
- **Time-lock (baseline)**:
  - High-risk actions (payments classified as `high-risk`, all key rotations, and all deploy/upgrade actions) MUST enforce a minimum delay starting when the action enters the execution-eligible state.
  ...
  - For non-chain settlement rails, this delay MUST be at least 24 hours, expressed in seconds in policy.
  ...
  - If an in-flight request is re-classified into a stricter risk category after it has entered the execution-eligible state, a new immutable time-lock-start reference MUST be appended (not mutating any prior reference) and the time-lock window for that stricter category MUST be restarted and measured from the timestamp or block height recorded on the re-classification event. Enforcement MUST use the most recent time-lock-start reference for the strictest category when checking time-lock expiry.
