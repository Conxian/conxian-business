...
- **Execution-eligible state**: the point in the approval lifecycle where all required approvals are recorded, all policy checks (including risk classification) have passed, and the action is eligible to start any required time-lock window. While in this state, the action MUST NOT be executed/signed/broadcast until all applicable time-lock windows have elapsed.
  ...
  - If no time-lock applies to a request under policy for its action type and effective risk classification (i.e., it is not a key rotation, deploy/upgrade action, or a payment classified as `high-risk`), a request MAY transition directly from the execution-eligible state to the execution-ready state immediately after eligibility checks pass.
...
- **Time-lock (baseline)**:
  ...
  - If policy for the effective risk classification is updated in a way that increases the required minimum delay while a request is in flight (even if its risk category does not change), the custody/approval system MUST append a new classification event for that request referencing the new policy hash/version and a new immutable time-lock-start reference (not mutating any prior reference). The time-lock window for that stricter delay MUST be restarted and measured from that new time-lock-start reference. Enforcement MUST use the most recently appended time-lock-start reference for the strictest effective risk classification when determining entry into (and maintaining) the execution-ready state.
