...
- **Execution-eligible state**: the point in the approval lifecycle where all required approvals are recorded, all policy checks (including risk classification) have passed, and the action is eligible to start any required time-lock window. While in this state, the action MUST NOT be executed/signed/broadcast until all applicable time-lock windows have elapsed.
  ...
  - The classification event that establishes execution-eligibility MUST record a policy hash/version equal to the request's policy hash/version at the moment of transition; if they do not match, the transition MUST be rejected and the request blocked.
  - Additional classification events MAY be appended later without mutating the request's policy hash/version or the eligibility snapshot; each classification event MUST record the policy hash/version used for that classification event, which MAY differ from the request's policy hash/version.
...
- **Execution-ready state**: the point where all applicable time-lock windows have elapsed under the effective risk classification, and the only remaining step is to execute/sign/broadcast the action.
  - For flows where policy defines no time-lock window for the effective risk classification, the transition from the execution-eligible state to the execution-ready state MAY be instantaneous (logically the same timestamp), because there are no applicable time-lock windows.
  - The transition into the execution-ready state (including cases with no time-lock, where it may share a timestamp with execution-eligible) MUST be inferable from tamper-evident records in the custody/approval system of record.
...
- **Time-lock (baseline)**:
  ...
  - When a high-risk action enters the execution-eligible state, a dedicated **time-lock-start reference** (block height for chain-settled assets or timestamp for non-chain rails) MUST be appended to the custody/approval system of record in a tamper-evident way and treated as an immutable field; any attempted in-place change for an in-flight request MUST cause the action to be blocked and audited. For non-chain rails, the time-lock-start reference MUST also record the identity of the hardened, auditable time source used, and that identity MUST be bound to the request for its lifetime so that all classification events and time-lock verification for that request use the same time source; if that bound time source cannot be reached or is rotated, the request MUST be treated as failed-closed. This time-lock-start reference MAY, but does not have to, equal the classification reference block height recorded earlier.
  ...
  - If the minimum delay required for the effective risk classification increases while a request is in flight but before it has entered the execution-ready state (even if its risk category does not change), enforcement MUST apply the stricter delay when determining entry into the execution-ready state. When determining whether a request may enter the execution-ready state, enforcement MUST derive the required minimum delay for the effective risk classification from the current policy source of truth; classification events MAY record which policy version was used to compute a classification, but MUST NOT be treated as fixing the numeric delay window.
