...
|---|---|---|
| `$47.96B` in 2026 to `$577.36B` in 2034 at `36.50%` CAGR | **Publisher value matched; application unsupported.** The values exactly match the Fortune Business Insights broad blockchain-technology landing-page forecast. The underlying methodology is paywalled and was not independently verified. | Cite only as a broad third-party category forecast. It is not, by itself, Conxian TAM. |
| `$738.62M` in 2026 to `$5.309B` in 2034 at `27.96%` CAGR | **Publisher value matched; application unsupported.** The values exactly match the Fortune Business Insights broad blockchain-interoperability landing-page forecast. The underlying methodology is paywalled and was not independently verified. | Cite only as a broad third-party category forecast. It is not, by itself, Conxian SAM. |
| CAGR calculations | **Recomputed.** `(577.36 / 47.96)^(1/8) - 1 = 36.45%`; `(5309 / 738.62)^(1/8) - 1 = 27.96%`. | The first published percentage is consistent after ordinary rounding; the second matches to two decimals. Mathematical consistency does not validate category selection or methodology. |
| `$30M-$75M` SOM | **Unsupported.** No sourced bottom-up customer count, price, adoption, sales-capacity, or conversion model was supplied. | Remove from public claims. A future restricted analysis needs explicit assumptions, sources, owners, and sensitivity ranges. |
| `40-50%` segment share and its use to derive SOM | **Unsupported and arithmetically inconsistent with the supplied SOM.** `40-50%` of `$738.62M` is approximately `$295M-$369M`, not `$30M-$75M`. | Do not use the percentage or derived SOM without a sourced denominator and a reproducible model. |
| Chainlink `18%`, Oracle `15%`, R3 `12%` | **Unsupported.** No source, denominator, geography, category definition, or observation date was supplied. | Remove. Dynamic usage or protocol metrics must not be relabeled as market share. |
